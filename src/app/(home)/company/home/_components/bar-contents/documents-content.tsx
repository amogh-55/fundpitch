...
const getViewerUrl = (fileUrl: string) => {
  ...
  // Alternatively, use Microsoft Office Online Viewer
...
const DocumentsContent = () => {
  ...
  } = api.company.getFinancialDocs.useQuery();
  const {
    data: corporateDocs,
    isLoading: isCorporateDocsLoading,
    refetch: refetchCorporateDocs,
  } = api.company.getCorporateDecks.useQuery();

  const { mutateAsync: deleteFinancialDoc } =
    api.company.deleteFinancialDoc.useMutation({
      onSuccess: () => {
        toast({
          description: "Financial document deleted successfully",
        });
        void refetchFinancialDocs();
      },
      onError: () => {
        toast({
          description: "Failed to delete financial document",
        });
      },
    });
  const { mutateAsync: deleteCorporateDeck } =
    api.company.deleteCorporateDeck.useMutation({
      onSuccess: () => {
        toast({
          description: "Corporate deck deleted successfully",
        });
        void refetchCorporateDocs();
      },
      onError: () => {
        toast({
          description: "Failed to delete corporate deck",
        });
      },
    });
                              ...
                              <AlertDialogFooter>
                                ...
                                <AlertDialogAction
                                  onClick={async () => {
                                    if (doc.file) {
                                      await Promise.all([
                                        deleteCorporateDeck({ id: doc.id }),
                                        handleDelete(doc.file),
                                      ]);
                                    } else {
                                      await deleteCorporateDeck({ id: doc.id });
                                    }
                                  }}
                                >
                                  Delete
                              ...
                              <AlertDialogFooter>
                                ...
                                <AlertDialogAction
                                  onClick={async () => {
                                    if (doc.file) {
                                      await Promise.all([
                                        deleteFinancialDoc({ id: doc.id }),
                                        handleDelete(doc.file),
                                      ]);
                                    } else {
                                      await deleteFinancialDoc({ id: doc.id });
                                    }
                                  }}
                                >
                                  Delete
